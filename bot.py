...
SCHEDULED_MESSAGES = [
    {'text': 'First message', 'post_at': (
            datetime.now() + timedelta(seconds=20)).timestamp(), 'channel': 'C01G8KM1A0J'},
    {'text': 'Second Message!', 'post_at': (
            datetime.now() + timedelta(seconds=30)).timestamp(), 'channel': 'C01G8KM1A0J'}
]
...
if __name__ == "__main__":
    ...
    ids = list_scheduled_messages('C01G8KM1A0J')
    print(ids)
    delete_scheduled_messages(ids, 'C01G8KM1A0J')
    app.run(debug=True)
